...
const findLinks = (markup, level) =>
  findItemsAtIndentLevel(markup, level).filter(isLink);
...
const processFolder = (folder, level) => {
  ...
  return cleanupObject({
    ...
    children: children
      ?.map((child) => processChild(child, level + 1))
      ?.filter(Boolean),
  });
};
 ...
 */
const bookmarksToJSON = (
  markup,
  { stringify = true, formatJSON = false, spaces = 2 } = {}
) => {
...
module.exports = { bookmarksToJSON };
